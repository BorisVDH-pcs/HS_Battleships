...
export default function Admin() {
  ...
  return (
    <div className="admin-split">
      ...
      <div className="admin">
      <section className="admin-overview-strip" aria-label="Command overview">
        <div>
          <span className="eyebrow">Command overview</span>
          <h2>{game ? game.name : 'Fleet command'}</h2>
          <p>{game ? STEP_HINT[game.status] : 'Create or select a game to begin.'}</p>
        </div>
        {game ? (
          <div className="admin-overview-metrics">
            <span><strong>{gameTeams.length}/2</strong> teams</span>
            <span><strong>{members.filter((m) => gameTeams.some((t) => t.id === m.team_id)).length}</strong> players</span>
            <span><strong>{tiles.length}/100</strong> tiles</span>
            {blocking.length > 0 && <span className="admin-overview-blocked"><strong>{blocking.length}</strong> blockers</span>}
          </div>
        ) : (
          <span className="admin-overview-hint">Select a game from Games to manage its board.</span>
        )}
      </section>
